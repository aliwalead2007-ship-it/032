    ...
    suspend fun generateScript(idea: String, styleDescription: String = "", contentType: String = "", contentTone: String = ""): List<Scene> = withContext(Dispatchers.IO) {
        ...
        return@withContext NetworkUtils.safeApiCall(
            ...
            val prompt = """
                ...
                معايير هوية قبس الإخراجية:
                1. المشهد الأول (الخطاف البصري Hook): يجب أن يكون جذاباً جداً وخاطفاً للانتباه مع عبارة عنوان قوية ونبرة تشويق وقورة.
                2. المشاهد الوسطى: ترتيل، تدبر، أو طرح دعوي عميق مع تدرج إضاءة سينمائية دافئة (Golden hour lighting, serene mosque geometry, natural landscapes).
                3. المشهد الختامي: رسالة أثر ملهمة وخاتمة هادئة (Call to Reflection).
                4. الدقة البصرية: كتابة وصف كل مشهد (description) باللغة الإنجليزية بتفاصيل بصرية عالية الدقة تناسب محركات توليد الفيديو والـ B-Roll السينمائي.
